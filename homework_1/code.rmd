...
```{r}
left <- data.table(svd.out$u,pData(bottomly.eset))
left
ggplot(left) + 
...
```{r}
left_five <- melt(left[,1:5, with=FALSE], variable.name = "Vector", value.name = "Loading")
left_five
```
```{r}
ggplot(left_five) + 
...
```{r}
clusters <- kmeans(edata, centers=5, nstart=20)
clusters
